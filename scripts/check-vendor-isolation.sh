#!/usr/bin/env bash
#
# CLAUDE.md rule 1: no vendor name outside src/lib/drivers/ or src/lib/publish/.
#
...
hits=$(grep -rl "$VENDORS" src --exclude-dir=drivers --exclude-dir=publish 2>/dev/null)

if [[ -n "$hits" ]]; then
  echo "FAIL: vendor names found outside src/lib/drivers/ and src/lib/publish/:"
  echo
...
ci_hits=$(grep -rli "$VENDORS" src --exclude-dir=drivers --exclude-dir=publish 2>/dev/null)
...
if [[ $status -eq 0 ]]; then
  echo "OK: no vendor names outside src/lib/drivers/ and src/lib/publish/."
fi
